...
export class Rarime {
  ...
  constructor(config: RarimeConfiguration) {
    if (config.userConfiguration.userPrivateKey.startsWith('0x')) {
      config.userConfiguration.userPrivateKey = config.userConfiguration.userPrivateKey.slice(
          2);
    }
  ...
  public async getDocumentStatus(passport: RarimePassport): Promise<DocumentStatus> {
    ...
    const contract = StateKeeper__factory.connect(
        this.config.contractsConfiguration.stateKeeperAddress, provider);
    ...
    const ZERO_BYTES_STRING = '0x' +
        Array.from(ZERO_BYTES).map((b) => b.toString(16)).join('');
  ...
  public async registerIdentity(passport: RarimePassport): Promise<String> {
    ...
    const txCallData = this.buildLiteRegisterCalldata(verifySodResponseParsed,
        proof, passport);
  ...
  private buildLiteRegisterCalldata(
      verifySodResponseParsed: any, proof: NoirZKProof,
      passport: RarimePassport): string {
    ...
    const passportStruct: RegistrationSimple.PassportStruct = {
      ...
      passportHash: '0x' +
          passport.getPassportHash().toString(16).padStart(64, '0'),
      verifier: verifySodResponseParsed.data.attributes.verifier, //if not work strip "0x" prefix
    };

    const txCallData = registrationSimpleContract.contractInterface.encodeFunctionData(
  ...
  private async generateLiteRegisterProof(
      hashLength: number, passport: RarimePassport): Promise<NoirZKProof> {
    ...
    let inputs = {
      dg1: NoirCircuitParams.formatArray(
          Array.from(passport.dataGroup1).
              map(byteValue => byteValue.toString()),
          false),
    ...
    if (Platform.OS === 'android') {
      inputs = {
        dg1: NoirCircuitParams.formatArray(Array.from(passport.dataGroup1).
            map(byteValue => byteValue.toString()), true),
        sk_identity: '0x' + this.config.userConfiguration.userPrivateKey,
      };
    }
  ...
  private async verifySodRequest(
      passport: RarimePassport, proof: NoirZKProof): Promise<Response> {
    const pubSignalsBuffers = proof.pub_signals.map(
    ...
    const verifySodRequest = {
      data: {
        id: '', type_field: 'register', attributes: {
          document_sod: {
            hash_algorithm: HashAlgorithm.fromOID(passport.extractDGHashAlgo()).
                toString(),
            signature_algorithm: SignatureAlgorithm.fromOID(
                passport.getSignatureAlgorithm()).toString(),
            signed_attributes: '0x' +
                Buffer.from(passport.extractSignedAttributes()).toString('hex'),
            encapsulated_content: '0x' +
                Buffer.from(passport.extractEncapsulatedContent()).
                    toString('hex'),
            signature: '0x' +
                Buffer.from(passport.extractSignature()).toString('hex'),
            pem_file: wrapPem(passport.getCertificate()),
            dg15: passport.dataGroup15 ? '0x' +
                Buffer.from(passport.dataGroup15).toString('hex') : '',
            aa_signature: passport.aaSignature ? '0x' +
