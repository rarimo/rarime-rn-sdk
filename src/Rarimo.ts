...
export class Rarime {
    ...
    constructor(config: RarimeConfiguration) {
        if (config.userConfiguration.userPrivateKey.startsWith("0x")) {
            config.userConfiguration.userPrivateKey = config.userConfiguration.userPrivateKey.slice(2);
        }
    ...
    public async getDocumentStatus(passport: RarimePassport): Promise<DocumentStatus> {
        const provider = new JsonRpcProvider(this.config.apiConfiguration.jsonRpcEvmUrl);

        const contract = StateKeeper__factory.connect(this.config.contractsConfiguration.stateKeeperAddress, provider);
    ...
    public async registerIdentity(passport: RarimePassport): Promise<String> {
        ...
        const txCallData = this.buildLiteRegisterCalldata(verifySodResponseParsed, proof, passport);
    ...
    private buildLiteRegisterCalldata(verifySodResponseParsed: any, proof: NoirZKProof, passport: RarimePassport): string {
        const registrationSimpleContract = createRegistrationSimpleContract(this.config.contractsConfiguration.registerSimpleContractAddress, new JsonRpcProvider(this.config.apiConfiguration.jsonRpcEvmUrl));
        ...
        const txCallData = registrationSimpleContract.contractInterface.encodeFunctionData("registerSimpleViaNoir", ["0x" + RarimeUtils.getProfileKey(this.config.userConfiguration.userPrivateKey), passportStruct, verifySodResponseParsed.data.attributes.signature, "0x" + proof.proof,]);

        return txCallData;
    }

    private async generateLiteRegisterProof(hashLength: number, passport: RarimePassport): Promise<NoirZKProof> {
        ...
        let inputs = {
            dg1: NoirCircuitParams.formatArray(Array.from(passport.dataGroup1).map(byteValue => byteValue.toString()), false),
    ...
    private async verifySodRequest(passport: RarimePassport, proof: NoirZKProof): Promise<Response> {
        const pubSignalsBuffers = proof.pub_signals.map((sig) => Buffer.from(sig, "hex"));
        ...
        const verifySodRequest = {
            data: {
                id: "", type_field: "register", attributes: {
                    document_sod: {
                        hash_algorithm: HashAlgorithm.fromOID(passport.extractDGHashAlgo()).toString(),
                        signature_algorithm: SignatureAlgorithm.fromOID(passport.getSignatureAlgorithm()).toString(),
                        signed_attributes: "0x" + Buffer.from(passport.extractSignedAttributes()).toString("hex"),
                        encapsulated_content: "0x" + Buffer.from(passport.extractEncapsulatedContent()).toString("hex"),
                        signature: "0x" + Buffer.from(passport.extractSignature()).toString("hex"),
                        pem_file: wrapPem(passport.getCertificate()),
                        dg15: passport.dataGroup15 ? "0x" + Buffer.from(passport.dataGroup15).toString("hex") : "",
                        aa_signature: passport.aaSignature ? "0x" + Buffer.from(passport.aaSignature).toString("hex") : "",
        ...
        const verifySodResponse = await fetch(this.config.apiConfiguration.rarimeApiUrl + "/integrations/incognito-light-registrator/v1/registerid", {
            method: "POST", headers: {
    ...
    private async sendRegisterLiteTransaction(txCallData: string): Promise<Response> {
        ...
        const liteRegisterResponse = await fetch(this.config.apiConfiguration.rarimeApiUrl + "/integrations/registration-relayer/v1/register", {
            method: "POST", headers: {
                "Content-Type": "application/json",
        ...
        if (!liteRegisterResponse.ok) {
            ...
            throw new Error(`HTTP error ${liteRegisterResponse.status}: ${JSON.stringify(errorData)}`);
        }
